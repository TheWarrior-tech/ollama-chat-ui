...
import { Plus, Trash2, X, Cpu, ChevronDown, MessageSquare, Share2, LogOut, User, Settings, Sun, Moon, Monitor, Pin, PinOff, Compass, FolderPlus } from 'lucide-react';
...
export default function Sidebar({ open, conversations, activeId, models, selectedModel, userName, onModelChange, onSelect, onNew, onDelete, onShare, onPin, onToggle, onSignOut }: Props) {
  ...
  const ConvoItem = ({ c }: { c: Conversation }) => (
    <div onClick={() => onSelect(c.id)}
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        {c.pinned && <Pin size={9} style={{ color: 'var(--accent)', flexShrink: 0 }} />}
        <span className="text-xs truncate">{c.title}</span>
      </div>
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all ml-1 flex-shrink-0">
        <button onClick={e => { e.stopPropagation(); onPin(c.id, !c.pinned); }} title={c.pinned ? 'Unpin' : 'Pin'}
          className="p-1.5 rounded-lg transition-all" style={{ color: 'var(--muted)' }}
          onMouseEnter={e => e.currentTarget.style.color='var(--accent-light)'} onMouseLeave={e => e.currentTarget.style.color='var(--muted)'}>
          {c.pinned ? <PinOff size={9} /> : <Pin size={9} />}
        </button>
        <button onClick={e => { e.stopPropagation(); onShare(c.id); }} title="Share"
          className="p-1.5 rounded-lg transition-all" style={{ color: c.shared ? '#60a5fa' : 'var(--muted)' }}
          onMouseEnter={e => e.currentTarget.style.color='#60a5fa'} onMouseLeave={e => e.currentTarget.style.color=c.shared?'#60a5fa':'var(--muted)'}>
          <Share2 size={9} />
        </button>
        <button onClick={e => { e.stopPropagation(); onDelete(c.id); }}
          className="p-1.5 rounded-lg transition-all" style={{ color: 'var(--muted)' }}
          onMouseEnter={e => e.currentTarget.style.color='#f87171'} onMouseLeave={e => e.currentTarget.style.color='var(--muted)'}>
          <Trash2 size={9} />
        </button>
      </div>
    </div>
  ...
  return (
    <aside className="w-[260px] flex-shrink-0 flex flex-col h-full transition-colors" style={{ background: 'var(--sidebar)', borderRight: '1px solid var(--border)' }}>
      <div className="flex items-center justify-between px-5 pt-5 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-gradient-accent flex items-center justify-center shadow-glow-sm"><Cpu size={15} className="text-white" /></div>
          <div>
      ...
      <div className="px-4 pb-3 flex gap-2">
        <button onClick={onNew} className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all"
          style={{ background: 'linear-gradient(135deg,rgba(99,102,241,0.12),rgba(139,92,246,0.08))', border: '1px solid rgba(99,102,241,0.2)', color: 'var(--accent-light)' }}>
          <Plus size={12} /> New Chat
        </button>
        <button onClick={() => router.push('/discover')} title="Discover"
          className="p-2 rounded-xl transition-all" style={{ background: 'var(--elevated)', border: '1px solid var(--border)', color: 'var(--muted)' }}
          onMouseEnter={e => e.currentTarget.style.color='var(--accent-light)'} onMouseLeave={e => e.currentTarget.style.color='var(--muted)'}>
          <Compass size={14} />
        </button>
      </div>
      ...
      <div className="px-4 pb-3">
        <div className="grid grid-cols-3 gap-1 p-1 rounded-xl" style={{ background: 'var(--elevated)', border: '1px solid var(--border)' }}>
          {([['dark','Dark',Moon],['light','Light',Sun],['system','Auto',Monitor]] as const).map(([v,l,Icon]) => (
            <button key={v} onClick={() => setTheme(v)}
              className="flex items-center justify-center gap-1 py-1.5 rounded-lg text-[10px] font-medium transition-all"
              style={{ background: theme===v ? 'var(--surface)' : 'transparent', color: theme===v ? 'var(--accent-light)' : 'var(--muted)', border: theme===v ? '1px solid var(--border-med)' : '1px solid transparent' }}>
              <Icon size={10} />{l}
            </button>
          ))}
        </div>
      ...
      <div className="px-5 pb-1"><span className="text-[9px] uppercase tracking-[0.15em] font-semibold" style={{ color: 'var(--muted)' }}>Chats</span></div>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-4 scrollbar-hide">
        ...
        {pinned.length > 0 && (
          <>
            <p className="text-[9px] uppercase tracking-wider px-2 py-1" style={{ color: 'var(--muted)' }}>Pinned</p>
      ...
      <div className="px-4 py-3" style={{ borderTop: '1px solid var(--border)' }}>
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <button onClick={() => router.push('/settings')} className="p-1.5 rounded-lg" style={{ color: 'var(--muted)' }} title="Settings">
            <Settings size={12} />
          </button>
          <button onClick={onSignOut} className="p-1.5 rounded-lg" style={{ color: 'var(--muted)' }}
            onMouseEnter={e => e.currentTarget.style.color='#f87171'} onMouseLeave={e => e.currentTarget.style.color='var(--muted)'}>
            <LogOut size={12} />
          </button>
        </div>
