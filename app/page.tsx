...
export default function Home() {
  ...
  // Chat prefs from localStorage
  const systemPrompt = typeof window !== 'undefined' ? localStorage.getItem('nc-system-prompt') || '' : '';
  const sendOnEnter = typeof window !== 'undefined' ? localStorage.getItem('nc-send-enter') !== 'false' : true;
      ...
      if (Array.isArray(data)) {
        const convos: Conversation[] = data.map((c:any) => ({
          ...
          messages:(c.messages||[]).map((m:any)=>({
            ...
            thinking:m.thinking??undefined, sources:m.sources??undefined,
            timestamp:new Date(m.createdAt).getTime(),
          })),
        }));
        setConversations(convos);
        // Support ?chat=id from shared URL redirect
  ...
  const sendMessage = async (content: string, attachments?: Attachment[]) => {
    ...
    const newTitle = activeConvo?.title === 'New Chat' ? (content||attachments?.[0]?.name||'Chat').slice(0,45) : activeConvo?.title;

    setConversations(p => p.map(c => c.id===activeId ? {
      ...c, messages:[...c.messages, userMsg, aMsg], title: newTitle || c.title
    } : c));

    if (activeConvo?.title === 'New Chat' && newTitle) {
      fetch(`/api/conversations/${activeId}`, {
        method:'PATCH', headers:{'Content-Type':'application/json'},
        body:JSON.stringify({title:newTitle}),
      }).then(r=>r.json()).then(updated => {
        // Update URL slug after title change
        if (updated.slug) window.history.replaceState({}, '', `/${updated.slug}/${activeId}`);
        setConversations(p => p.map(c => c.id===activeId ? {...c, slug:updated.slug} : c));
      }).catch(()=>{});
    }
    ...
    try {
      ...
      if (webSearch) {
        ...
        if (sources.length) setConversations(p => p.map(c => c.id===activeId ? {...c,messages:c.messages.map(m=>m.id===tempAiId?{...m,sources}:m)} : c));
      }

      const priorMessages = (activeConvoRef.current?.messages.filter(m=>m.id!==tempAiId&&m.id!==tempUserId)||[]).map(m=>({role:m.role,content:m.content}));
      ...
      // Pass custom system prompt to API — it will prepend it
      const res = await fetch('/api/chat', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body:JSON.stringify({ model:selectedModel, messages, systemPrompt: systemPrompt||undefined }),
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            setConversations(p => p.map(c => c.id===activeId ? {...c,messages:c.messages.map(m=>m.id===tempAiId?{...m,content:fullContent,thinking:fullThinking||undefined}:m)} : c));
          } catch {}
        }
      }
      ...
      if (e?.name!=='AbortError') {
        setConversations(p=>p.map(c=>c.id===activeId?{...c,messages:c.messages.map(m=>m.id===tempAiId&&m.content===''?{...m,content:'⚠️ Cannot reach Ollama. Run `ollama serve`.'}:m)}:c));
      }
    ...
    <div className="flex h-screen overflow-hidden" style={{background:'var(--bg)',color:'var(--text)'}}>
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full relative z-10">
        <header className="flex items-center justify-between px-6 py-3.5 flex-shrink-0 glass" style={{borderBottom:'1px solid var(--border)'}}>
          ...
          <div className="flex items-center gap-2">
            <button onClick={()=>router.push('/discover')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all"
              style={{background:'var(--elevated)',border:'1px solid var(--border)',color:'var(--text-dim)'}}>
              <Compass size={11}/>Discover
            </button>
            {activeId&&(
              <button onClick={()=>handleShare(activeId)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all"
                style={{background:'var(--elevated)',border:'1px solid var(--border)',color:'var(--text-dim)'}}>
            ...
            <button onClick={()=>setVoiceMode(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all"
              style={{background:'var(--elevated)',border:'1px solid var(--border)',color:'var(--text-dim)'}}>
              <Mic size={11}/>Voice
            </button>
        ...
        {activeConvo?.messages?.length === 0 || !activeId
