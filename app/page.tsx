...
import { Mic, Compass, Search, Download, X } from 'lucide-react';
    ...
    fetch('/api/conversations').then(r=>r.json()).then(data => {
      if (Array.isArray(data)) {
        const convos: Conversation[] = data.map((c:any) => ({
          id:c.id, title:c.title, slug:c.slug, model:selectedModel, shared:c.shared, pinned:c.pinned||false,
          messages:(c.messages||[]).map((m:any)=>({ id:m.id, role:m.role, content:m.content, thinking:m.thinking??undefined, sources:m.sources??undefined, timestamp:new Date(m.createdAt).getTime() })),
        }));
  ...
  const sendMessage = useCallback(async (content: string, attachments?: Attachment[]) => {
    ...
    if (currentConvo?.title === 'New Chat' && newTitle) {
      fetch(`/api/conversations/${targetId}`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body:JSON.stringify({title:newTitle}) })
        .then(r=>r.json()).then(updated => {
          if (updated.slug) window.history.replaceState({}, '', `/${updated.slug}/${targetId}`);
          setConversations(p => p.map(c => c.id===targetId ? {...c, slug:updated.slug, title:updated.title} : c));
        }).catch(()=>{});
    }
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body:JSON.stringify({ model:selectedModel, messages, systemPrompt: getSystemPrompt()||undefined }),
      ...
      if (e?.name!=='AbortError') {
        setConversations(p=>p.map(c=>c.id===targetId?{...c,messages:c.messages.map(m=>m.id===tempAiId&&m.content===''?{...m,content:'⚠️ Cannot reach Ollama. Run `ollama serve`.'}:m)}:c));
      }
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{background:'var(--bg)',color:'var(--text)'}}>
      ...
      <Sidebar
        ...
        onModelChange={setSelectedModel} onSelect={selectChat}
        onNew={newChat} onDelete={async id=>{
          await fetch(`/api/conversations/${id}`,{method:'DELETE'});
          setConversations(p=>p.filter(c=>c.id!==id));
          if (activeId===id) setActiveId(conversations.find(c=>c.id!==id)?.id??null);
        }}
        onShare={handleShare} onPin={handlePin}
      ...
      <div className="flex flex-col flex-1 min-w-0 h-full relative z-10">
        ...
        <header className="flex items-center justify-between px-5 py-3 flex-shrink-0 glass" style={{borderBottom:'1px solid var(--border)'}}>
          <div className="flex items-center gap-3">
            {!sidebarOpen && (
              <button onClick={()=>setSidebarOpen(true)} className="p-2 rounded-xl" style={{color:'var(--muted)'}}>
          ...
          <div className="flex items-center gap-1.5">
            {/* Search chats */}
            <button onClick={()=>setSearchOpen(true)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium transition-all"
              style={{background:'var(--elevated)',border:'1px solid var(--border)',color:'var(--text-dim)'}} title="⌘K">
              <Search size={11}/>Search
              <kbd className="hidden sm:inline text-[9px] px-1 py-0.5 rounded" style={{background:'var(--card)',color:'var(--muted)'}}>⌘K</kbd>
            </button>
            {/* Export */}
            {activeConvo && activeConvo.messages.length > 0 && (
              <div className="relative">
                <button onClick={()=>setExportOpen(o=>!o)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium transition-all"
            ...
            {/* Share */}
            {activeId && (
              <button onClick={()=>handleShare(activeId)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium transition-all"
                style={{background:'var(--elevated)',border:'1px solid var(--border)',color:'var(--text-dim)'}}>
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>Share
              </button>
            )}
            {/* Voice */}
            <button onClick={()=>setVoiceMode(true)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-medium transition-all"
              style={{background:'var(--elevated)',border:'1px solid var(--border)',color:'var(--text-dim)'}}>
              <Mic size={11}/>Voice
            </button>
            {/* Discover toggle */}
            <button onClick={()=>setDiscoverOpen(o=>!o)}
              ...
              style={{
                background: discoverOpen ? 'linear-gradient(135deg,rgba(99,102,241,0.15),rgba(139,92,246,0.1))' : 'var(--elevated)',
                border: discoverOpen ? '1px solid rgba(99,102,241,0.3)' : '1px solid var(--border)',
                color: discoverOpen ? 'var(--accent-light)' : 'var(--text-dim)',
              }}>
              <Compass size={11}/>{discoverOpen ? 'Hide' : 'Discover'}
            </button>
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase" style={{color:'var(--muted)'}}>NeuralChat</span>
          </div>
        </header>

        {/* Chat + Discover side by side */}
        <div className="flex flex-1 min-h-0">
          {/* Chat area */}
          <div className="flex flex-col flex-1 min-w-0">
            {showWelcome
              ? <WelcomeScreen userName={(session?.user as any)?.name||''} onSuggestion={s=>sendMessage(s)} />
              : <ChatWindow messages={activeConvo?.messages??[]} isStreaming={isStreaming} onRegenerate={()=>{}} />
            }
          ...
          {/* RIGHT: Discover panel — slides in */}
          {discoverOpen && (
            <div className="w-[340px] flex-shrink-0 h-full overflow-hidden transition-all" style={{borderLeft:'1px solid var(--border)'}}>
              <DiscoverPanel
                onAskAI={(prompt) => { sendMessage(prompt); setDiscoverOpen(false); }}
